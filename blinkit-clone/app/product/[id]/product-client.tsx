...
export default function ProductClient({ product: initialProduct }: { product: Product | null }) {
    ...
    useEffect(() => {
        if (initialProduct) {
            ...
            if (initialProduct.variants && initialProduct.variants.length > 0) {
                setSelectedVariant(initialProduct.variants[0].id)
            }
            setLoading(false)
            fetchRelatedProducts(initialProduct)
        } else {
            // Fallback for client-side fetch if initialProduct is missing (should not happen with server component)
            // fetchProduct()
        }
    }, [initialProduct])
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <main className="container py-6">
                ...
                <div className="mt-12">
                    <SmartRecommendations context="product" productId={product.id} categoryId={product?.categoryId} />
                </div>
            </main>
